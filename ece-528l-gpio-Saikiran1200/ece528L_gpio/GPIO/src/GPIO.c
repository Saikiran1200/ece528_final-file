...
void LED_Pattern_4()
{
    ...
    for (int led_count = 1; led_count <= 0x80; led_count = led_count*2)
    {
        PMOD_8LD_Output(led_count);
        Clock_Delay1ms(200);
        uint8_t switch_status = Get_PMOD_SWT_Status();
...
void LED_Pattern_5()
{
    ...
    for (int led_count = 0x80; led_count >= 0x01; led_count = led_count/2)
    {
        PMOD_8LD_Output(led_count);
        Clock_Delay1ms(200);
        uint8_t switch_status = Get_PMOD_SWT_Status();
...
void Johnson_Counter()
{
    ...
    LED2_Output(RGB_LED_OFF);
    for (int led_count = 0x01; led_count <= 0xFF00; led_count = (led_count << 1) | (led_count & ~(led_count >> 7)) )
    {
        PMOD_8LD_Output(led_count);
        Clock_Delay1ms(200);
        uint8_t switch_status = Get_PMOD_SWT_Status();
